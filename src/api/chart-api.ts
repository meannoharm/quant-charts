...
import { ScatterSeriesOptions, ScatterSeriesPartialOptions } from './../model/series-options';
...
export class ChartApi implements IChartApi, DataUpdatesConsumer<SeriesType> {
	...
	public addScatterSeries(options: ScatterSeriesPartialOptions = {}): ISeriesApi<'Scatter'> {
		options = migrateOptions(options);

		const strictOptions = merge(clone(seriesOptionsDefaults), scatterStyleDefault, options) as ScatterSeriesOptions;
		const series = this._chartWidget.model().createSeries('Scatter', strictOptions);

		const res = new SeriesApi<'Scatter'>(series, this, this);
		this._seriesMap.set(res, series);
		this._seriesMapReversed.set(series, res);
