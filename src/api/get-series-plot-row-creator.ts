...
import { BarData, CandlestickData, HistogramData, isWhitespaceData, LineData, ScatterData, SeriesDataItemTypeMap } from './data-consumer';
...
function getColoredLineBasedSeriesPlotRow(time: TimePoint, index: TimePointIndex, item: LineData | HistogramData | ScatterData): Mutable<SeriesPlotRow<'Line' | 'Histogram' | 'Scatter'>> {
	const val = item.value;

	const res: Mutable<SeriesPlotRow<'Line' | 'Histogram' | 'Scatter'>> = { index, time, value: [val, val, val, val] };
...
const seriesPlotRowFnMap: SeriesItemValueFnMap = {
	...
	Scatter: wrapWhitespaceData(getColoredLineBasedSeriesPlotRow),
};
