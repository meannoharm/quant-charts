...
export class SeriesBarColorer {
	...
	public barStyle(barIndex: TimePointIndex, precomputedBars?: PrecomputedBars): BarColorerStyle {
		...
		switch (targetType) {
			...
			case 'Scatter':
				return this._scatterStyle(seriesOptions as ScatterStyleOptions, barIndex, precomputedBars);
		}
	...
	private _scatterStyle(scatterStyle: ScatterStyleOptions, barIndex: TimePointIndex, precomputedBars?: PrecomputedBars): BarColorerStyle {
		const currentBar = ensureNotNull(this._findBar(barIndex, precomputedBars)) as SeriesPlotRow<'Scatter'>;

		return {
			...emptyResult,
			barColor: currentBar.color ?? scatterStyle.color,
		};
	}
